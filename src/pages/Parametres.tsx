...
export default function Parametres() {
    ...
    const handleSave = async () => {
        ...
        // 2. loading
        setSaving(true);
        toast.loading('Enregistrement en cours...', { id: 'settings-save' });
        try {
            // 3. validate (no validation needed for toggles)
            // 4. call mutation (simulated)
            await new Promise(r => setTimeout(r, 800));
            // 5. update state (already in local state)
            // 6. success toast
            toast.success('Paramètres enregistrés avec succès', { id: 'settings-save' });
            // 7. auto-dismiss (via sonner)
        } catch {
            // 8. catch error
            toast.error('Erreur lors de l\'enregistrement', { id: 'settings-save' });
        } finally {
            setSaving(false);
        }
    };
