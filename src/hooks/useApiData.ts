    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const mockData: InitiativePTMApi[] = INITIATIVES_PTM.map((init) => ({
                ...
                ministere_nom: init.ministereNom || 'Ministère',
                ministere_sigle: init.ministereSigle || '',
                direction_id: init.directionId || '',
                direction_nom: init.directionNom || '',
                annee: init.annee || 2026,
                ...
                statut: init.statut,
                soumis_par: init.soumisParId || undefined,
                soumis_par_nom: init.soumisParNom || undefined,
                date_soumission: init.dateSoumission || undefined,
                motif_rejet: init.motifRejet || undefined,
                created_at: init.creeLe || new Date().toISOString(),
                updated_at: init.modifieLe || new Date().toISOString(),
            }));
