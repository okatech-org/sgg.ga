      ...
      <div className="container mx-auto px-4">
        ...
        <FadeInView className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-foreground mb-4">
            Ce qu'ils disent de <span className="text-an">SGG Digital</span>
          </h2>
              ...
              <CarouselContent className="-ml-4">
                {testimonials.map((testimonial, index) => (
                  <CarouselItem key={index} className="pl-4 md:basis-1/2 lg:basis-1/3">
                    ...
                    >
                      <div className="h-full neu-card p-6 flex flex-col">
                        {/* Quote Icon */}
                        <Quote className="h-8 w-8 text-an/30 mb-4" />
                        ...
                        <div className="flex gap-1 mb-4">
                          {Array.from({ length: testimonial.rating }).map((_, i) => (
                            <Star key={i} className="h-4 w-4 fill-an text-an" />
                          ))}
                        </div>
          ...
          <div className="border-t pt-12">
            ...
            <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12">
              {partners.map((partner, index) => (
                <motion.div 
                  key={index}
                  className={`${partner.opacity} hover:opacity-100 transition-opacity duration-300 text-lg font-serif font-semibold text-foreground`}
